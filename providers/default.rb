...
def load_current_resource
  @current_resource = Chef::Provider::Ark.new(@new_resource.name, run_context)
end
...
def untar_cmd_cherry_pick(sub_cmd)
  ...
  cmd = Mixlib::ShellOut.new(%Q{#{tar_cmd} --no-anchored -O -#{sub_cmd} '#{new_resource.release_file}' #{new_resource.creates} > '#{dest}';})
  cmd.run_command
  cmd.error!
end
